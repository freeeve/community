...
public abstract class GraphDatabaseSettings
{
    ...
    public static final StringSetting tx_manager_impl = new StringSetting( "tx_manager_impl", ANY,
            "Must be class name of TransactionManager implementation" );
    ...
    @Default(FALSE)
    public static final BooleanSetting intercept_deserialized_transactions = new BooleanSetting(
    ...
    @Default("127.0.0.1")
    public static final StringSetting remote_logging_host = new StringSetting( "remote_logging_host", ANY,
    ...
    public static final GraphDatabaseSetting.IntegerSetting threshold_for_logging_rotation = new GraphDatabaseSetting
            .IntegerSetting( "logging.threshold_for_rotation", "Must be valid number", 1, null );

    // Indexing
    @Description("Controls the auto indexing feature for nodes. Setting to false shuts it down unconditionally, " +
            "while true enables it for every property, " +
    ...
    @Description("A list of property names (comma separated) that will be indexed by default. This applies to Nodes " +
            "only.")
    public static final StringSetting node_keys_indexable = new StringSetting( "node_keys_indexable", ANY,
            "Must be a comma-separated list of keys to be indexed" );

    @Description("Controls the auto indexing feature for relationships. Setting to false shuts it down " +
            "unconditionally, while true enables it for every property, " +
            "subject to restrictions in the configuration.")
    @Default(FALSE)
    ...
    @Description(" A list of property names (comma separated) that will be indexed by default. This applies to " +
            "Relationships only.")
    public static final StringSetting relationship_keys_indexable = new StringSetting( "relationship_keys_indexable",
            ANY, "Must be a comma-separated list of keys to be indexed" );

    // Lucene settings
    @Description("Integer value that sets the maximum number of open lucene index searchers.")
    @Default(Integer.MAX_VALUE + "")
    public static IntegerSetting lucene_searcher_cache_size = new IntegerSetting( "lucene_searcher_cache_size",
            "Must be a number", 1, null );
    ...
    @Default(FALSE)
    public static final BooleanSetting intercept_committing_transactions = new BooleanSetting(
    ...
    public static final StringSetting keep_logical_logs = new StringSetting( "keep_logical_logs", ANY,
            "Must be 'true'/'false' or of format '<number><optional unit> <type>' for example '100M size' for " +
    ...
    @Default("20M")
    public static final NumberOfBytesSetting nodestore_mapped_memory = new NumberOfBytesSetting( "neostore.nodestore" +
            ".db.mapped_memory" );

    @Description("The size to allocate for memory mapping the property value store.")
    @Default("90M")
    public static final NumberOfBytesSetting nodestore_propertystore_mapped_memory = new NumberOfBytesSetting(
    ...
    @Default("1M")
    public static final NumberOfBytesSetting nodestore_propertystore_index_mapped_memory = new NumberOfBytesSetting(
    ...
    @Default("1M")
    public static final NumberOfBytesSetting nodestore_propertystore_index_keys_mapped_memory = new
            NumberOfBytesSetting( "neostore.propertystore.db.index.keys.mapped_memory" );

    @Description("The size to allocate for memory mapping the string property store.")
    @Default("130M")
    public static final NumberOfBytesSetting strings_mapped_memory = new NumberOfBytesSetting( "neostore" +
            ".propertystore.db.strings.mapped_memory" );

    @Description("The size to allocate for memory mapping the array property store.")
    @Default("130M")
    public static final NumberOfBytesSetting arrays_mapped_memory = new NumberOfBytesSetting( "neostore.propertystore" +
            ".db.arrays.mapped_memory" );

    @Description("The size to allocate for memory mapping the relationship store.")
    @Default("100M")
    public static final NumberOfBytesSetting relationshipstore_mapped_memory = new NumberOfBytesSetting( "neostore" +
            ".relationshipstore.db.mapped_memory" );

    @Default("100")
    public static final IntegerSetting relationship_grab_size = new IntegerSetting( "relationship_grab_size",
            "Must be a number" );
    ...
    @Default("120")
    public static final IntegerSetting string_block_size = new IntegerSetting( "string_block_size",
            "Must be a number", 1, null );
    ...
    @Description("Specifies the block size for storing arrays. This parameter is only honored when the store is " +
            ...
            "i.e., 8 bytes.")
    @Default("120")
    public static final IntegerSetting array_block_size = new IntegerSetting( "array_block_size", "Must be a number",
            1, null );
    ...
    @Default("")
    public static final GraphDatabaseSetting<String> forced_kernel_id = new StringSetting( "forced_kernel_id",
            "[a-zA-Z0-9]*",
            "Invalid kernel identifier." );

    // TODO: This should be in enterprise, but we currently have code depending on this in community
    @Default(FALSE)
    public static final GraphDatabaseSetting.BooleanSetting online_backup_enabled = new GraphDatabaseSetting
            .BooleanSetting( "online_backup_enabled" );

    // TODO: This should be in enterprise, but we currently have code depending on this in community
    @Default("6362")
    public static final GraphDatabaseSetting.PortSetting online_backup_port = new GraphDatabaseSetting.PortSetting(
            "online_backup_port" );
    ...
    public static final GraphDatabaseSetting.DirectorySetting store_dir = new GraphDatabaseSetting.DirectorySetting(
            "store_dir", true, true );
    ...
    @Default("neostore")
    public static final GraphDatabaseSetting.FileSetting neo_store = new GraphDatabaseSetting.FileSetting(
            "neo_store", store_dir, true, true );
    ...
    public static final GraphDatabaseSetting.FileSetting logical_log = new GraphDatabaseSetting.FileSetting(
            "logical_log", store_dir, true, true );
    ...
    public static final GCRMemoryUsageSetting relationship_cache_size = new GCRMemoryUsageSetting(
    ...
    @Description("The fraction of the heap (1%-10%) to use for the base array in the node cache (when using the 'gcr'" +
            " cache).")
    @Default("1.0")
    public static final FloatSetting node_cache_array_fraction = new FloatSetting( "node_cache_array_fraction",
            "Must be a valid fraction", 1.0f, 10.0f );

    @Description("The fraction of the heap (1%-10%) to use for the base array in the relationship cache (when using " +
            "the 'gcr' cache).")
    @Default("1.0")
    public static final FloatSetting relationship_cache_array_fraction = new FloatSetting(
            "relationship_cache_array_fraction", "Must be a valid fraction", 1.0f, 10.0f );

    @Description("The minimal time that must pass in between logging statistics from the cache (when using the 'gcr' " +
            "cache).")
    @Default("60s")
    public static final TimeSpanSetting gcr_cache_min_log_interval = new TimeSpanSetting(
    ...
    public static final class GCRMemoryUsageSetting extends NumberOfBytesSetting implements
            org.neo4j.graphdb.factory.GraphDatabaseSetting.DefaultValue
    {
